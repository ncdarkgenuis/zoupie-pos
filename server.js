...
app.post('/inventory/add-bulk', async (req, res) => {
    ...
    try {
        for (const item of items) {
            // Checks for a product row specifically matching this account email address
            ...
            if (existing) {
                // Safely increments stock quantity for returning shop accounts
                ...
                // Dynamically inserts a standalone product record for any new testing account profile
                await supabase
                    .from('products')
                    .insert([{
                        owner_email: email,
